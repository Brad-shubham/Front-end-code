...
const DemographicScore = () => {
    ...
    const { selectedListing, clients, constants,actionRequired, clientMetricData, activeDemoMetric, clientPsychoPoints, clientTotalPoints, helper, tiersList } = useSelector((state) => state?.dataPack)
    usePrompt(constants?.POPUP_MODAL_MESSAGE,actionRequired)
   
    useEffect( async() => {
        helper?.setTitle();
        /** get client saved metric data */
        await dispatch(getClientMetricData(selectedListing?.value, constants?.METRIC_TYPE?.DEMOGRAPHIC))

        await dispatch(getClientScoreData(selectedListing?.value,constants?.METRIC_TYPE?.DEMOGRAPHIC))
                    ...
                    <table className="table table-hover mb-0">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" colSpan="5">Demographics Metrics Values</th>
    ...
    function renderMetric() {
        return clientMetricData?.map((option, index) => {
            ...
            return (
                <>
                    <th scope="col" key={index?.toString()}>{option?.metric_title}</th>
                </>
            )
        })
    }
    ...
    function renderClient  () {
        ...
        return clients?.map(client => {
            ...
            return (
                <tr>
                    <td><span className='float-start ps-1'>{client?.name}</span> <button className="bg-none border-0 dlt-client float-end" type="button" onClick={(el) => handleUserdeleteEvent(el, client)}><i className="fa fa-trash"></i></button></td>
                    {
